...
func NewDropout(rate tensai.Float) *Dropout {
...
func (d *Dropout) Forward(input *tensai.Matrix) (*tensai.Matrix, error) {
	...
	if cap(d.mask) < len(input.Data) {
		d.mask = make([]tensai.Float, len(input.Data))
	}
	d.mask = d.mask[:len(input.Data)]
	out := d.fwdBuf(input.Rows, input.Cols)
	for i, v := range input.Data {
		if tensai.Float(d.rng.Float64()) < keep {
			d.mask[i] = 1 / keep
		} else {
			d.mask[i] = 0
		}
...
func (d *Dropout) Backward(gradOutput *tensai.Matrix) (*tensai.Matrix, error) {
	if d.mask == nil {
...
func (d *Dropout) Params() (*tensai.Matrix, []tensai.Float)       { return nil, nil }
func (d *Dropout) Grads() (*tensai.Matrix, []tensai.Float)        { return nil, nil }
func (d *Dropout) SetParams(*tensai.Matrix, []tensai.Float) error { return nil }
