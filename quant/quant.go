...
func padRows8(xus [][]uint8, sxs []tensai.Float, rowLen, cols int) ([][]uint8, []tensai.Float, *tensai.Matrix) {
	pxus := make([][]uint8, 8)
	psxs := make([]tensai.Float, 8)
	...
	return pxus, psxs, tensai.NewMatrix(8, cols)
}
...
// column: W[i][j] ~= tensai.Float(q_ij) * Scale[j]. Rows are stored in
...
// Quantize quantizes column-wise, symmetric around zero with
...
func Quantize(m *tensai.Matrix) *QMatrix {
	quads := (m.Rows + 3) / 4
	q := &QMatrix{
		Rows:     m.Rows,
		Cols:     m.Cols,
		Q:        make([]int8, ((m.Cols+q4Tile-1)/q4Tile)*quads*4*q4Tile+32),
		Scale:    make([]tensai.Float, m.Cols),
...
func quantizeActs(x []tensai.Float) (xu []uint8, sx tensai.Float) {
	padded := (len(x) + 3) &^ 3
...
func quantizeActsScalar(x []tensai.Float, xu []uint8) tensai.Float {
	var maxAbs tensai.Float
	for _, v := range x {
		if v < 0 {
			v = -v
...
func (q *QMatrix) MatVec(x, out []tensai.Float) error {
...
func (q *QMatrix) MatMul(x, out *tensai.Matrix) error {
...
func qmatmulRows8Generic(out *tensai.Matrix, xus [][]uint8, sxs []tensai.Float, r0 int, qw []int8, scale []tensai.Float, colSum64 []int32, cols, lo, hi int) {
	var acc [8][]int32
	for r := range acc {
	...
	for r := 0; r < 8; r++ {
		...
		for j := lo; j < hi; j++ {
			o[j] = tensai.Float(acc[r][j-lo]-colSum64[j]) * scale[j] * sxs[r]
		}
	}
}
...
func qmatvecColsGeneric(out []tensai.Float, xu []uint8, sx tensai.Float, qw []int8, scale []tensai.Float, colSum64 []int32, cols, lo, hi int) {
	...
	for j := lo; j < hi; j++ {
		out[j] = tensai.Float(acc[j-lo]-colSum64[j]) * scale[j] * sx
	}
}
