...
func NewQ8GMatrix(rows, cols, group int) *Q8GMatrix {
	...
	return &Q8GMatrix{
		...
		Scale:    make([]tensai.Float, ((cols+q4Tile-1)/q4Tile)*groups*q4Tile),
...
func (q *Q8GMatrix) MatVec(x, out []tensai.Float) error {
...
func (q *Q8GMatrix) MatMul(x, out *tensai.Matrix) error {
...
func q8gMatvecColsGeneric(out []tensai.Float, xu []uint8, sx tensai.Float, qw []int8, scale []tensai.Float, colSum64 []int32, group, cols, lo, hi int) {
	clear(out[lo:hi])
	quads := len(xu) / 4
...
func q8gMatmulRows8Generic(out *tensai.Matrix, xus [][]uint8, sxs []tensai.Float, r0 int, qw []int8, scale []tensai.Float, colSum64 []int32, group, cols, lo, hi int) {
	for r := 0; r < 8; r++ {
