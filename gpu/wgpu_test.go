...
func openTestGPU(t *testing.T) *Device {
	t.Helper()
	g, err := Open()
	if err != nil {
...
func TestGPUAdapterSelection(t *testing.T) {
	...
	x, err := tensai.NewTensorFromSlice([]tensai.Float{1, 2, 3, 4}, 2, 2)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGPUTensorChain(t *testing.T) {
	...
	wantAB, err := tensai.MatMul(a, b)
	if err != nil {
		t.Fatal(err)
	}
	want, err := tensai.MatMul(wantAB, c)
	if err != nil {
		t.Fatal(err)
	}
	for i := range want.Data {
...
func cpuSoftmaxLast(x *tensai.Tensor) {
...
func TestGPUAttention(t *testing.T) {
	...
	want, err := tensai.MatMul(scores, v)
	if err != nil {
		t.Fatal(err)
	}
	if !dims.Same(got.Shape, want.Shape) {
...
func cpuAttention(t *testing.T, q, k, v *tensai.Tensor) *tensai.Tensor {
	t.Helper()
	kt, err := k.Transpose()
	if err != nil {
		t.Fatal(err)
	}
	s, err := tensai.MatMul(q, kt)
	if err != nil {
		t.Fatal(err)
	}
	s.Scale(1 / kernels.SqrtF(tensai.Float(q.Shape[len(q.Shape)-1])))
	cpuSoftmaxLast(s)
	out, err := tensai.MatMul(s, v)
	if err != nil {
		t.Fatal(err)
	}
	return out
...
func TestGPUMultiHeadAttention(t *testing.T) {
	...
	want := tensai.NewTensor(batch, seq, d)
	for b := 0; b < batch; b++ {
		for h := 0; h < heads; h++ {
			slice := func(x *tensai.Tensor, rows int) *tensai.Tensor {
				s := tensai.NewTensor(rows, dh)
				for r := 0; r < rows; r++ {
					for c := 0; c < dh; c++ {
...
func cpuCausalAttention(t *testing.T, q, k, v *tensai.Tensor) *tensai.Tensor {
	t.Helper()
	seq, d := q.Shape[0], q.Shape[1]
	seqKV := k.Shape[0]
	out := tensai.NewTensor(seq, d)
...
func TestGPUMatMulLarge(t *testing.T) {
	...
	for i := range want.Data {
		// f32 accumulation order differs between CPU and Device.
...
func TestGPUQ8MatMul(t *testing.T) {
	...
	for _, c := range []struct{ m, rows, cols int }{
		...
		// Reference: the same dequantized weights on the CPU. The Device
	...
	q := quant.Quantize(tensai.RandomMatrix(8, 8, rng))
	gq, err := g.UploadQ8(q)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGPUQ4MatMul(t *testing.T) {
	...
	for _, c := range []struct{ m, rows, cols int }{
		...
		// activations (the Device path skips the 7-bit activation step).
		for r := 0; r < c.m; r++ {
			for j := 0; j < c.cols; j++ {
				var want float64
				for i := 0; i < c.rows; i++ {
	...
	q, err := quant.Quantize4(tensai.RandomMatrix(8, 8, rng))
	if err != nil {
		t.Fatal(err)
	}
	gq, err := g.UploadQ4(q)
