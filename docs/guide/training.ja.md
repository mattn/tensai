...
Dense、活性化、LayerNorm、Conv2D、MaxPool2D、Dropout、BatchNorm にグラフ形があります。後ろ 2 つの学習時/推論時の切り替えは `SetTraining` で、層スタックにおける `Fit` と `Predict` に相当します。Embedding はまだ無く、それを含むモデルは `Fit` と違う学習を黙って行うのではなくエラーになります。
