...
デバイスにカーネルがある演算 (3 つの積、繰り返しオペランドを許す要素ごとの演算、ReLU/tanh/sigmoid/GELU とその勾配、ブロードキャストの集約、Adam) はデバイスで走り、それ以外は CPU にフォールバックして、その演算に必要なぶんだけ持ち帰ります。つまりこれらの演算だけで組んだグラフはデバイスから出ませんし、`LayerNorm` や `Embed` (まだカーネルがありません) を使うグラフはその箇所で往復して続行します。
