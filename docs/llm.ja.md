...
の下に組織名とモデル名で置かれます。`-model Qwen/Qwen3-4B-Instruct-2507` なら
`Qwen/Qwen3-4B-Instruct-2507` です。別の組織が同じ名前のモデルを出しても
ファイルを共有しません。以前のバージョンで取得したものはモデル名だけの
ディレクトリにあり、取得元のリポジトリに対しては引き続きそれを使います。
それ以外の場所に置きたいなら、そこへ取得してからパスで指定してください。
...
まだ持っていないマシンでそのまま打てる名前です。置き場所も組織名の
ディレクトリの下なので、`models rm Qwen/Qwen3-0.6B` はそのモデルだけを消し、
同じ組織の他のモデルには触りません。モデル名だけのディレクトリにある古い
ダウンロードは、記録した取得元のリポジトリ名で並び、`models rm` はどちらの
形でも受けます。組織のディレクトリを素の名前で指定した場合は、中身ごと
消さずにエラーにします。手で置いたものはディレクトリ名のままです。
