...
されます。GPU デコードも通ります。Radeon 780M の実測で
CPU 14.2 tok/s に対して 15.7 tok/s、プレフィルはほぼ同じです。差が小さいので
電源状態の方が効きます — バッテリー駆動では同じ GPU が 3.1 tok/s まで落ちる一方、
CPU は 13.6 tok/s を保ちます。
