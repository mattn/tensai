...
されます。テンプレート自体はツール呼び出しの形式を持っていますが未実装なので、
`tools` 付きのリクエストは慣習を持たないファミリーと同じく拒否します。repack
キャッシュと GPU デコードはこのアーキテクチャを対象外にしています。
