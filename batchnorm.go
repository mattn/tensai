...
type BatchNorm struct {
	...
	xhat   *Matrix // cached normalized input from the last training forward
	invStd []Float
...
func (b *BatchNorm) Init(inputCols int, _ *rand.Rand) (int, error) {
	...
	b.gamma = NewMatrix(1, inputCols)
	for i := range b.gamma.Data {
		b.gamma.Data[i] = 1
	}
	b.beta = make([]Float, inputCols)
	b.gradGamma = NewMatrix(1, inputCols)
	b.gradBeta = make([]Float, inputCols)
	b.runMean = make([]Float, inputCols)
	b.runVar = make([]Float, inputCols)
...
func (b *BatchNorm) Forward(input *Matrix) (*Matrix, error) {
	...
	m := Float(input.Rows)
	b.xhat = ensureMatrix(b.xhat, input.Rows, cols)
	if cap(b.invStd) < cols {
		b.invStd = make([]Float, cols)
	} else {
...
func (b *BatchNorm) Backward(gradOutput *Matrix) (*Matrix, error) {
	...
	m := Float(gradOutput.Rows)
	out := b.bwdBuf(gradOutput.Rows, cols)
	for c := 0; c < cols; c++ {
		var sumG, sumGX Float
...
func (b *BatchNorm) Params() (*Matrix, []Float) { return b.gamma, b.beta }
func (b *BatchNorm) Grads() (*Matrix, []Float)  { return b.gradGamma, b.gradBeta }
...
func (b *BatchNorm) extraState() map[string][]Float {
...
func (b *BatchNorm) setExtraState(state map[string][]Float) error {
