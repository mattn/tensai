...
func TestKNNTwoClusters(t *testing.T) {
	...
	const perClass = 50
	train := NewMatrix(2*perClass, 2)
	targets := NewMatrix(2*perClass, 1)
	for i := 0; i < perClass; i++ {
		train.Set(2*i, 0, Float(rng.NormFloat64())*0.5-2)
		train.Set(2*i, 1, Float(rng.NormFloat64())*0.5)
		targets.Data[2*i] = 0
		train.Set(2*i+1, 0, Float(rng.NormFloat64())*0.5+2)
		train.Set(2*i+1, 1, Float(rng.NormFloat64())*0.5)
	...
	test, err := NewMatrixFromSlice(2, 2, []Float{-2, 0, 2, 0})
	if err != nil {
		t.Fatal(err)
	}
...
func TestKNNOneNeighborMemorizes(t *testing.T) {
	...
	train := RandomMatrix(30, 4, rng)
	targets := NewMatrix(30, 1)
	for i := range targets.Data {
		targets.Data[i] = Float(i % 3)
	}
	knn := NewKNN(1)
	if err := knn.Fit(train, targets); err != nil {
		t.Fatal(err)
...
func TestKNNValidation(t *testing.T) {
	knn := NewKNN(0)
	if err := knn.Fit(NewMatrix(3, 2), NewMatrix(3, 1)); err == nil {
		t.Error("k=0 should be rejected")
	}
	knn = NewKNN(2)
	if _, err := knn.Predict(NewMatrix(1, 2)); err == nil {
		t.Error("predict before fit should fail")
	}
	if err := knn.Fit(NewMatrix(3, 2), NewMatrix(2, 1)); err == nil {
		t.Error("target row mismatch should be rejected")
	}
	tgt := NewMatrix(3, 1)
	tgt.Data[1] = 1.5
	if err := knn.Fit(NewMatrix(3, 2), tgt); err == nil {
