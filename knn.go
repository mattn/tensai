...
type KNN struct {
	...
	trainT  *Matrix // transposed training inputs (features x samples)
	sqNorm  []Float // squared norm per training sample
	labels  []int
	classes int
}
...
func (k *KNN) Fit(inputs, targets *Matrix) error {
	...
	k.trainT = inputs.T()
	k.sqNorm = make([]Float, inputs.Rows)
	...
	for r := 0; r < inputs.Rows; r++ {
		...
		if cls < 0 || Float(cls) != targets.Data[r] {
...
func (k *KNN) Predict(inputs *Matrix) (*Matrix, error) {
	...
	train := k.trainT.Cols
	out := NewMatrix(inputs.Rows, k.classes)
	...
	for off := 0; off < inputs.Rows; off += chunk {
		...
		view := &Matrix{Rows: n, Cols: inputs.Cols, Data: inputs.Data[off*inputs.Cols : (off+n)*inputs.Cols]}
		prod, err := Dot(view, k.trainT) // n x train
		if err != nil {
			return nil, err
		}
		for r := 0; r < n; r++ {
