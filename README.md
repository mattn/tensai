...
- **Sequential models** - stack layers and run `Compile` -> `Fit` / `FitStep` -> `Predict`; `Graph()` builds the same model as an autograd graph instead — every layer but `Embedding` — which trains the very same weights and can run on a GPU
