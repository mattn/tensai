...
`_example/qwen` does the same for a modern instruction-tuned model: Qwen2.5-0.5B-Instruct, with RMSNorm, rotary position embeddings, grouped-query attention, and a SwiGLU MLP, its BF16 checkpoint loaded through the same safetensors reader and its ChatML template applied around the prompt. Dimensions come from config.json and sharded checkpoints load through their index.json, so other Qwen2 sizes work unchanged — and so do the Llama architecture (the same block without attention biases: `-repo HuggingFaceTB/SmolLM2-135M-Instruct` runs SmolLM2 at ~50 tok/s, its Digits pre-tokenizer handled by the tokenizer package) **Qwen3**, which adds per-head QK-norm and an explicit head_dim (`-repo Qwen/Qwen3-0.6B` chats out of the box, answering directly by default and reasoning in a `<think>` block first with `-think`), **SmolLM3**, whose only twist is skipping RoPE on every fourth layer (`-repo HuggingFaceTB/SmolLM3-3B` runs the 3B), **Gemma 3** — sliding-window attention on five of every six layers, sandwich norms, a gelu-tanh gate, per-layer rope bases, and a SentencePiece tokenizer — via its GGUF (`-gguf gemma-3-1b-it-Q8_0.gguf -q8`, `-gpu` included), and **Phi-3**, whose GGUF ships q/k/v and gate/up pre-fused exactly the way the runtime wants them (`-gguf Phi-3-mini-4k-instruct-q4.gguf -q4`) — plus the **DeepSeek-R1 distills**, which are stock qwen2/llama blocks wearing DeepSeek's turn markers: the loader spots the template, switches chat markers, and the model reasons in its `<think>` block out of the box (`-gguf DeepSeek-R1-Distill-Qwen-1.5B-Q4_K_M.gguf -q4`), and **mixture-of-experts** models: a router picks each token's experts, whose small FFNs repack per expert straight from the GGUF's 3D tensors — Qwen's qwen2moe/qwen3moe (Qwen1.5-MoE-A2.7B, 14B total 2.7B active, answers at ~9 tok/s from a 20-second load) and OpenAI's **gpt-oss**, whose native MXFP4 expert blocks stream through their own table-lookup kernel with attention sinks, alternating 128-token sliding windows, YaRN rope, and the o200k tokenizer all along for the ride: gpt-oss-20b reasons in its harmony analysis channel and answers on this same 15GB machine; with `-q8`/`-q4` each weight quantizes as it loads and its float32 copy dies immediately, so the full-precision model never has to fit in memory — and the layers load in parallel (with the quantizer itself splitting columns across CPUs), which turns a 55-second 1.5B load into 14 seconds. On a 15GB machine that ladder looks like: 0.5B at ~40 tok/s with `-q8` and a 1.5B Q4_K_M at ~25 with `-q4` (the tiled integer kernels, measured on native Windows), and Qwen2.5-**7B**-Instruct — 15GB of BF16 shards, int4-quantized on the fly during a two-minute load into ~6GB resident — answering correctly at 3.5 tok/s. Prompts feed through a batched prefill: `QMatrix.MatMul` streams the weights once per block of eight token rows (and `Q4Matrix.MatMul` once per four) instead of once per token, which cuts the wait before the first generated token by around 6x. `-gguf` loads a llama.cpp checkpoint instead — one downloaded file carrying config, tokenizer, and weights through `encoding/gguf` (the q/k rope permutation of llama-architecture conversions is undone during load): `-gguf qwen2.5-0.5b-instruct-q8_0.gguf -q8` chats with nothing else on disk. Q8_0 and Q4_0 checkpoints skip the float32 detour entirely: the blocks repack from the memory-mapped file into grouped int8/int4 matrices with pure integer work (Q4_0's nibble encoding matches tensai's exactly), keeping llama.cpp's own 32-row scales — a 3B Q8_0 GGUF opens in 5 seconds instead of 32. K-quant checkpoints stream the same way, the whole Q4_K/Q5_K/Q6_K family under both widths: under `-q4`, Q4_K's nibbles copy raw into a min-form int4 matrix that keeps its per-32-value scale *and* min (the asymmetric kernels fold the offset through the activation group sums) while five- and six-bit spans renormalize onto nibbles with integer rounding; under `-q8`, Q6_K widens losslessly to grouped int8 and the min-form types requantize onto an int8 grid many times finer than their own — so `-gguf qwen2.5-1.5b-instruct-q4_k_m.gguf -q4` loads in about 3 seconds instead of 8, with llama.cpp's own quantization intact (`-requant` restores the float detour, trading the much slower load for about 10% more decode speed from its coarser symmetric tables). `-draft` points at a smaller same-family model for speculative decoding (greedy only): the draft proposes a few tokens, one batched pass of the big model verifies them, and rejections roll the caches back, so the output is exactly what the big model alone would produce — Qwen2.5-**7B** with the 0.5B drafting goes from 1.2 to 1.6 tok/s. The default `-spec 3` fills one four-row verification block; the draft only pays off when the target is much larger than it. Sampling (`-temp` above 0) restricts itself to the nucleus: `-topp 0.9` keeps the smallest probability-sorted set of tokens holding 90% of the mass, so the long tail where repetition loops and derailments live never gets a lottery ticket. `-chat` turns it into a multi-turn conversation on stdin — the KV cache carries the whole dialogue, so each turn only processes its own tokens — and `-serve :8080` exposes the same model as an OpenAI-compatible `/v1/chat/completions` endpoint (messages array, SSE streaming, usage counts), so any OpenAI client pointed at it chats with a pure-Go model:
