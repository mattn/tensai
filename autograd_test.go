...
func checkParamGrad(t *testing.T, param *Matrix, build func() (*Node, *Node), name string) {
	t.Helper()
	loss, p := build()
...
func TestAutogradOps(t *testing.T) {
	...
	b := RandomMatrix(1, 4, rng)
	other := RandomMatrix(5, 4, rng)
...
func TestAutogradSoftmaxCE(t *testing.T) {
	...
	w := RandomMatrix(3, 4, rng)
	x := RandomMatrix(6, 3, rng)
	target := NewMatrix(6, 1)
	for i := range target.Data {
		target.Data[i] = Float(rng.Intn(4))
	}
...
func TestAutogradTrainsXOR(t *testing.T) {
	...
	inputs, err := NewMatrixFromSlice(4, 2, []Float{0, 0, 0, 1, 1, 0, 1, 1})
	if err != nil {
		t.Fatal(err)
	}
	targets, err := NewMatrixFromSlice(4, 1, []Float{0, 1, 1, 0})
	if err != nil {
		t.Fatal(err)
	}

	w1 := Param(RandomMatrix(2, 8, rng))
	b1 := Param(NewMatrix(1, 8))
	w2 := Param(RandomMatrix(8, 1, rng))
	b2 := Param(NewMatrix(1, 1))
	trainer := NewTrainer(NewAdam(0.05), w1, b1, w2, b2)

	forward := func(x *Matrix) *Node {
