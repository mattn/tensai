...
func (s *SGD) Step(idx int, weights, gradW *Matrix, bias, gradB []Float) {
	if s.velW[idx] == nil {
		s.velW[idx] = NewMatrix(weights.Rows, weights.Cols)
	}
	if s.velB[idx] == nil {
...
func (a *Adam) Step(idx int, weights, gradW *Matrix, bias, gradB []Float) {
	if a.mW[idx] == nil {
		a.mW[idx] = NewMatrix(weights.Rows, weights.Cols)
		a.vW[idx] = NewMatrix(weights.Rows, weights.Cols)
		a.mB[idx] = make([]Float, len(bias))
		a.vB[idx] = make([]Float, len(bias))
	}
	a.t++
