...
func makeDataset(t *testing.T, n int) *Dataset {
	t.Helper()
	in := tensai.NewMatrix(n, 3)
	tgt := tensai.NewMatrix(n, 1)
	for i := 0; i < n; i++ {
		for c := 0; c < 3; c++ {
			in.Set(i, c, tensai.Float(i*10+c))
		}
		tgt.Data[i] = tensai.Float(i)
	}
	ds, err := New(in, tgt)
	if err != nil {
		t.Fatal(err)
...
func TestDatasetShuffleKeepsRowsPaired(t *testing.T) {
	...
	for i := 0; i < ds.Len(); i++ {
		...
		seen[id] = true
		for c := 0; c < 3; c++ {
			if ds.Inputs.At(i, c) != tensai.Float(id*10+c) {
...
func TestDatasetBatches(t *testing.T) {
	ds := makeDataset(t, 10)
	var visited []int
	var lastBatch *tensai.Matrix
	err := ds.Batches(3, nil, func(in, tgt *tensai.Matrix) error {
	...
	if err := ds.Batches(11, nil, func(_, _ *tensai.Matrix) error { return nil }); err == nil {
...
func TestDatasetStandardize(t *testing.T) {
	...
	ds, err := New(in, tensai.NewMatrix(200, 1))
	if err != nil {
		t.Fatal(err)
	}
	...
	other := tensai.NewMatrix(1, 2)
	other.Data[0], other.Data[1] = mean[0], mean[1]
	ods, _ := New(other, tensai.NewMatrix(1, 1))
